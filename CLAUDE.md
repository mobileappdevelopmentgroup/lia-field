...
- [ ] 🔴 **`inspections` is readable by anon on the live database.** This was recorded as applied on 2026-07-29; it is not in effect. Verified 2026-08-23 by querying the live REST API with the publishable key: `GET /rest/v1/inspections` returns 200 with 1,724 rows including `tech_name`. The key is public by design (it is in the deployed certificate site), so this exposes every inspection record rather than the curated `ladder_inspections_public` view. `supabase/02_inspections.sql` contains the correct `REVOKE`; it needs to actually be run. Applying the migrations — either route in `docs/APPLY-MIGRATIONS.md` — fixes it, and `supabase/test/run-reset.sh` asserts it stays fixed.
