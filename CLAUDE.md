...
- [ ] 🔴 **`inspections` is readable by anon on the live database.** This was recorded as applied on 2026-07-29; it is not in effect. Verified 2026-08-23 and **re-confirmed 2026-08-24** by querying the live REST API with the publishable key: `GET /rest/v1/inspections` returns 200 with 1,724 rows including `tech_name`. The key is public by design (it is in the deployed certificate site), so this exposes every inspection record rather than the curated `ladder_inspections_public` view. `supabase/02_inspections.sql` contains the correct `REVOKE`; it needs to actually be run. Applying the migrations fixes it — follow **`docs/SHIP-RUNBOOK.md`** (Route B). ⚠️ **Do not use Route A.** The live data is production, not test data: 1,724 rows, 1,619 of them another tech's, backing certificates that customers look up by the serial on a physical ladder tag. `supabase/test/run-reset.sh` asserts the fix stays in place.
